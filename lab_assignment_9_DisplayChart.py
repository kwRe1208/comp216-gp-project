...
class DisplayChartApp:
    ...
    def __init__(
        ...
        self._label = Label(self._frame, text="Data Range:")
        self._label.grid(row=0, column=0, sticky="w")
        self._entry = Spinbox(
            self._frame,
            width=10, font=("Arial", 12),
            from_=0,
            to=len(data_points) - 1,
            increment=1,
            validate="key",
            validatecommand=(self._frame.register(self._validate_on_input), "%P"),
    ...
    def draw_chart(self, start_index: int = 0, end_index: int = None):
        ...
        self._chart.clear()
        self._chart.draw_bars(self._data_points[start_index:end_index], color="lightgreen")
    ...
    def _draw_chart_on_input(self):
        try:
            start_index = int(self._entry.get())
